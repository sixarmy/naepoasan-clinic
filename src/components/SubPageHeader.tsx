...
export default function SubPageHeader({ category, title }: { category: string; title: string }) {
  ...
  return (
    <section className="bg-[#f4fbf7] px-4 pb-[60px] pt-[58px] sm:pb-[70px] sm:pt-[70px] lg:px-5 lg:pb-[110px] lg:pt-[100px]">
      <div className="mx-auto flex max-w-[1400px] flex-col items-start">
        ...
        <h1 className="text-[28px] font-black leading-tight tracking-[-0.055em] text-[#3d9b6a] sm:text-[40px] lg:text-[58px]">{title}</h1>
      </div>
    </section>
  );
}
...
function Dropdown({
  ...
  return (
    <div className="relative" id={`simc-${id}`}>
      <button type="button" onClick={setOpen} className="flex items-center gap-1.5 text-[13px] font-semibold text-[#444] transition-colors hover:text-[#3d9b6a] sm:text-[15px] lg:text-[16px]">
        {label}
      ...
      <div className={`absolute left-0 top-[calc(100%+10px)] z-40 min-w-[170px] rounded-xl border border-[#eef2f6] bg-white py-2 shadow-[0_15px_35px_rgba(0,0,0,0.1)] transition-all duration-300 lg:top-[calc(100%+14px)] lg:min-w-[190px] ${open ? "visible translate-y-0 opacity-100" : "invisible translate-y-2 opacity-0"}`}>
        {links.map((link) => (
          <Link key={link.label} href={link.href} className={`block px-4 py-2.5 text-[13px] transition-colors hover:bg-slate-50 hover:text-[#3d9b6a] lg:px-5 lg:py-3 lg:text-[14px] ${link.active ? "bg-slate-100 font-black text-[#3d9b6a]" : "text-[#555]"}`}>
            {link.label}
          </Link>
        ))}
      </div>
