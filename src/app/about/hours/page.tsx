      ...
      <section className="bg-white px-5 py-14 sm:py-20">
        <div className="mx-auto max-w-4xl">
          <ScrollReveal delay={120}>
            <div className="overflow-hidden rounded-3xl border border-[#E2E8F0] bg-[#f4fbf7]">
              <div className="border-b border-[#E2E8F0] bg-white px-7 py-6 sm:px-9">
                <p className="text-[12px] font-black uppercase tracking-[0.16em] text-[#3d9b6a]">Clinic Hours</p>
                <h3 className="mt-2 text-2xl font-black tracking-[-0.04em] text-[#3d9b6a]">진료시간 안내</h3>
                ...
                <ul className="border-y border-b-[#eeeeee] border-t-[#111] border-t-2 bg-white px-4 py-3 sm:px-6">
                  {clinic.hours.map((row) => (
                    <li key={row.label} className="flex items-center border-b border-[#f2f2f2] py-3 last:border-b-0">
                      ...
                      <span className="flex-1 text-[15px] font-bold leading-6 text-[#3d9b6a] sm:text-[17px]">{row.value}</span>
                    </li>
                  ))}
                </ul>
                <div className="mt-5 rounded-2xl bg-white px-5 py-4 text-sm leading-7 text-[#666]">
                  <p>ㆍ{clinic.hoursNotice}</p>
                  <p>ㆍ대표전화: <a href={`tel:${clinic.phone}`} className="font-black text-[#3d9b6a]">{clinic.phoneLabel}</a></p>
